...
def qinxu_jiaoyie_oneday(trade_data_oneday):
    ...
    # 涨停板
    zhangtingban_df = select_zhangting_or_dieting_by_tradedf(df_format_float, '涨停')
    # 涨停个股交易额
    zhangtingban_df_amount = '% .0f ' % (zhangtingban_df.amount.sum() / 100000)
    n_zhangtingban_df = len(zhangtingban_df)

    # 炸板
    zhaban_df = select_zhangting_or_dieting_by_tradedf(df_format_float, '炸板')
    zhaban_df_amount = '% .0f ' % (zhaban_df.amount.sum() / 100000)
    n_zha_df = len(zhaban_df)

    # 跌停
    dieting_df = select_zhangting_or_dieting_by_tradedf(df_format_float, '跌停')
    dieting_df_amount = '% .0f ' % (dieting_df.amount.sum() / 100000)
    n_dieting_df = len(dieting_df)
...
def days_jiaoyie(endday, day_num):
    ...
    print('交易日，大盘交易额，涨停交易额，涨停数，炸板交易额，炸板数，跌停交易额，跌停数')
